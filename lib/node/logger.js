 ...
 * logger.js - basic logger for wmcc_node.
 */
...
const {fs, util, co, Lock} = Core.utils;
...
Logger.prototype.log = function log(level, module, args) {
  ...
  this.writeBrowser(level, module, args);
...
/**
 * Write log to the browser.
 * @param {String} level
 * @param {String|null} module
 * @param {Object[]} args
 */

Logger.prototype.writeBrowser = function writeBrowser(level, module, args) {
  const name = Logger.levelsByVal[level];
  ...
  if (module !== 'net') console.log(msg); // fix this later
