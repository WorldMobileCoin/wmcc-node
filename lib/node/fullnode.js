 ...
 * fullnode.js - full node for wmcc_node.
 */

'use strict';

const Core = require('wmcc-core');
const HTTPServer = Core.http.Server;
const {Chain} = Core.blockchain;
const {Fees, Mempool} = Core.txmempool;
const {Pool} = Core.net;
const {Miner} = Core.mining;
const {RPC} = Core.http;
const {WalletDB} = Core.wallet;
const {Stratum} = Core.stratum;
const {Auth} = require('wmcc-credential').Credential;
...
function FullNode(options) {
  ...
  // Client needs access to the node. //check ??working for real node??
  this.client = new NodeClient(this);

  // WalletDB needs access to the node client.
  this.walletdb = new WalletDB({
    network: this.network,
    logger: this.logger,
    workers: this.workers,
    client: this.client,
    // node: this, // add real node client
    // client: this, // check this if needed
    prefix: this.config.prefix,
  ...
  // set info for browser
  this.info = new Info(this);

  // hook/attach node to browser event
...
/**
 * Attach/Hook an event listener to spesific module.
 * @param {String} event
 * @param {String} module
...
FullNode.prototype._on = async function _on(event, module, el) {
  return await this.attach.on(event, module, el);
};

/**
 * Attach/Hook an event emitter to spesific module.
 * @param {String} event
 * @param {String} module
 * return {Object} Event
 */

FullNode.prototype._emit = async function _emit(event, module) {
  return await this.attach.emit(event, module);
};

/**
 * Attach/Hook a function spesific module.
...
FullNode.prototype._call = async function _call(func, module, options) {
  if (options && Object.keys(options).length)
    return await this[module][func](...Object.values(options));
  return await this[module][func]();
};
